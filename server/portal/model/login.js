...
var loginModel = {
  procesLogin: async (req, res) => {
    try {
      ...
      let query = await req.db.query(
        `select * from tbl_students where email='${email}' and password='${encryptPwd}'`,
        'login'
      );
      if (query.length > 0) {
        profile = query[0];
        if (profile.status > 0) {
          ...
          res.cookie('user', profile, {
            maxAge: 9000000 * 27 * 360,
            httpOnly: true,
          });
        } else {
          profile.status =
            'Your account still is in pending state, Please send mail to the Admin to activate your account';
        }
      } else {
