...
const CreateStdProfile = memo(props => {
  ...
  const onFinish = async e => {
    ...
    await axios
      .post(`/events/api/saveStudentProfile`, {
        data: {
          ...formData,
          ...(() => {
  ...
  useEffect(() => {
    ...
    if (id != null && categoryData.categoryList.loading == false) {
      eventEditObj = _(categoryData.categoryList.data)
        .filter(val => val.studentid == parseInt(id))
        .value();
      ...
      if (Object.keys(eventEditObj).length == 0) {
        ...
        formInit.studentcat_json &&
          ...
          setcategoryList(
            Array.from(
              {length: formInit.studentcat_json.length},
              (val, key) => key
            )
          );
      ...
      <div className="_apifilter_subheader">
        <div className="_details">
          <div className="_title">
            {' '}
            <SafetyCertificateTwoTone twoToneColor="#52c41a" /> Create Student
            Profile{' '}
          </div>
          <div className="_subTitle">
            create student profile with dynamic fields
          </div>
        </div>
