...
import { getSiteNotificationResult } from '../../store/actions';
...
const ListScrollMsg = (props) => {
  ...
  const tagData = useSelector(state => state.notification);
  ...
  if (tags != undefined) {
    for (let i = 0; i < tags.length; i++) {
      tagDatas.push({
        key: (i + 1),
        id: tags[i].id,
        notifictions: tags[i].notifictions,
      });
    }
  }
  ...
  const EditableCell = ({
    ...
    const inputNode = <Input id="tag" placeholder="Notifictions Name"/>;
    return (<>
      <td {...restProps}>
        {editing ? (
          <Form.Item
  ...
  const columns = [
    {
      title: 'Notifictions',
      dataIndex: 'notifictions',
      editable: true,
      width: '80%',
      sorter: (a, b) => lib.NumberStringSort(a, b, 'notifictions'),
      render: (text, record) => {
        return (<>
          {text}
  ...
  return <>
        <div className="_apifilter_subheader">
            <div className="_details">
                <div className="_title"> <SafetyCertificateTwoTone twoToneColor="#52c41a" /> Site notification </div>
                <div className="_subTitle">Site notification details</div>
            </div>
