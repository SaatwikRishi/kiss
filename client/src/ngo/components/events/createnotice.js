...
const CreateTag = memo((props) => {
    ...
    const onFinish = async (e) =>{
        console.log(e);
        let formData = {
            notifictions: e.notifictions
        };        
        setloading(true);
        await axios.post(`/events/api/saveNotification`, { data: formData }).then(res => {
    ...
    return <>
        <div className="_apifilter_subheader">
            <div className="_details">
                <div className="_title"> <SafetyCertificateTwoTone twoToneColor="#52c41a" /> Create Notice </div>
                <div className="_subTitle">create notice </div>
            </div>
    ...
    return <>
        <div className="category_box">
            <div className="category_item">
                <Form.Item  hasFeedback={true} name= {['notifictions', formIndex ,'name']} label="name" rules={[{ required: true, message: 'Please fill!' }]}>
