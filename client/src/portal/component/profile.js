...
const ProfilePage = props => {
  ...
  const onFinish = async e => {
    ...
    let formData = _(e)
      ...
      .value();
    setloading(true);
    formData['fields'] = undefined;
    const temp = formData['student_json'];
    const obj = [];
    obj.push(temp);
    formData['student_json'] = obj;
    await axios
      .post(`/events/api/saveStudentProfile`, {data: formData})
      .then(res => {
  ...
  useEffect(() => {
      ...
      ...(() => {
        if (eventEditObj.student_json) {
          try {
            ...
            studentcat_json =
              studentcat_json && studentcat_json.length
                ? studentcat_json
                : null;

            if (studentcat_json) {
              return {studentcat_json: studentcat_json[0]};
            }
          } catch (error) {}
        }
      })(),
    ...
    formInit.studentcat_json &&
      formInit.studentcat_json.length &&
      setcategoryList(
        Array.from({length: formInit.studentcat_json.length}, (val, key) => key)
      );

    console.log('FORM INIT');
    console.log(formInit);
    console.log('Form no mo init');
    formInit['student_json'] = formInit['fields'];
            ...
            >
              <div className="category_box_basic">
                ...
                <div className="category_item">
                  ...
                  >
                    <Select placeholder="-District-">
                      {districtData.districts.map(val => (
                ...
                <div className="category_item">
                  <Form.Item
                    hasFeedback={true}
                    name={'tags'}
                    label="Areas of Intrest"
                    rules={[{required: true, message: 'Please fill!'}]}
                  >
                    <Select
                      mode="tags"
                      style={{width: '100%'}}
                      placeholder="Areas of Intrest"
                    >
                      {children}
                    </Select>
                  </Form.Item>
              ...
              <section>
                {student.student_json ? (
                  <CategoryForm
                    {...{student, form, fUpdateTrigger, eventsData}}
                  />
                ) : (
                  ''
                )}
...
const CategoryForm = props => {
  ...
  const dateFormat = 'YYYY-MM-DD';
  console.log('LOGGING STUDETNCAT');

  const parsed = JSON.parse(props.student.student_json);
  return (
    <>
      <Divider style={{margin: '20px 0'}} />
      <div
        className="category_box_basic"
        style={{
          display: 'grid',
        ...
        {catgoryJson.map(val => {
          ...
          if (val.input_type == 'datepicker') {
            ...
            if (parsed[val.name]) {
              const n = val.name;
              let obj = {};
              obj[n] = moment(parsed[val.name]);
              form.setFieldsValue({student_json: obj});
            }
