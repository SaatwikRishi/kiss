...
const ListingView = props => {
  ...
  const unregisterEvent = (eventid, studentid) => {
    setloading(true);
    axios
      .post('/events/api/deleteEventForms', {data: {eventid, studentid}})
      .then(function(res) {
  ...
  const applyForEvent = (eventid, student, eventDetails) => {
    ...
    if (!student.studentid) {
      ...
      eventDetails.event_json == '' ||
      eventDetails.event_json == null
    ) {
      setloading(true);
      axios
        .post(`/events/api/saveStudentEventForm`, {
          data: {
            form_json: '',
  ...
  const addComments = e => {
    ...
    axios.post('/api/addComments', body).then(res => {
      notification.info({
        message:
          'Your comments has been sent for moderation. Will update you once the admin review your comments',
      });
      form.resetFields();
    });
  };
                ...
                <Col sm={24} md={18}>
                  <Card bordered className="details">
                    <div className="EventImg">
                      <div
                        className={`img ${
                          eventDetails.gallery == null ? 'blankImg' : ''
                        }`}
                        style={{
                          backgroundImage: `url(${eventDetails.gallery})`,
                        }}
                      ></div>
                    </div>
                    <Divider />
                      ...
                      comments.data.map(rec => (
                        <div>
                          <Comment
                            ...
                            datetime={
                              <Tooltip
                                title={moment(rec.com_date).format(
                                  'YYYY-MM-DD HH:mm:ss'
                                )}
                              >
                    ...
                    <Form form={form} layout="vertical" onFinish={addComments}>
                      <Form.Item label="Add comments" name="comments" required>
                        <TextArea rows={4} />
                      </Form.Item>
                      <Form.Item>
                ...
                <Col sm={24} md={6} className="categories">
                  {forms.length > 0 ? (
                    <>
                      <Button
                        ...
                        onClick={() =>
                          unregisterEvent(ListingViewId, student.studentid)
                        }
                      >
                        UnRegister
                      </Button>
                    ...
                    <>
                      <Button
                        ...
                        onClick={() =>
                          applyForEvent(ListingViewId, student, eventDetails)
                        }
                      >
                        Apply / Register
                      </Button>
                  ...
                  >
                    {event_json && (
                      <EventDetailsForm
                        {...{
                          formFields: event_json,
                          eventId: ListingViewId,
                          studentId: student.studentid,
                        }}
                      />
                    )}
                  ...
                  <Card
                    title={
                      <div className="catd_title_right">
                        <b>Similar Events</b>
                      </div>
                    }
                  >
      ...
      renderItem={item => (
        <List.Item>
          <List.Item.Meta
            ...
            title={
              <Link to={`/listing/${item.eventid}`} title={item.event_name}>
                {item.event_name}
              </Link>
            }
          />
                  ...
                  <div className="category_item">
                    ...
                    >
                      <DatePicker.MonthPicker
                        ...
                        style={{width: '100%'}}
                        disabledDate={date =>
                          moment(date).isAfter(moment.now())
                        }
                      />
                    </Form.Item>
                  </div>
              ...
              if (val.input_type == 'upload') {
                template = (
                  <div className="category_item">
                    ...
                    >
                      <FireBaseFileUpload
                        {...{formName: keyName, form, fUpdateTrigger}}
                      />
                    </Form.Item>
                  </div>
                );
...
const EventInfo = props => {
  ...
  return (
    <>
      <div
        className="event_details_wrapper"
        style={{
